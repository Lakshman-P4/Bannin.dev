...
Bannin is a universal monitoring platform that watches everything you run — your machine, your LLMs, your cloud notebooks, your coding tools — and puts it all on your phone. It's not just a system resource monitor. It's a health dashboard for your entire digital workflow.

**Who it's for**: everyone in the digital era. Not just developers — anyone who uses AI. If you use ChatGPT and your conversation starts giving worse answers, Bannin tells you why and helps you fix it. If you're a student running code on Colab, Bannin warns you before your session dies. If you're a writer using Claude and want to know when to start a fresh chat, Bannin watches for you. If you're a developer calling LLM APIs, Bannin tracks your usage. If you're running a build and walk away, Bannin pings your phone when it's done. The audience is anyone who interacts with AI or compute — students, writers, researchers, creators, business professionals, hobbyists, and yes, developers too.
...
1. **Python Agent** (`bannin/`) — installed via `pip install bannin`, runs on the user's machine or inside cloud notebook kernels (Colab, Kaggle). Collects system metrics (CPU, RAM, GPU, disk) using `psutil`, `pynvml`, and ML-specific hooks (`torch.cuda`, `tqdm` interception). Exposes a local REST API at `localhost:8420`.
...
4. **MCP Server** (`bannin-mcp`) — exposes Bannin as a Model Context Protocol server so AI tools (Claude Code, Cursor, Windsurf) can query system status via standardized tool calls. Built on Anthropic's MCP Python SDK, JSON-RPC 2.0, stdio transport.
...
The Python agent runs inside the notebook kernel (`!pip install bannin`). Monitors Colab-specific concerns:
- **Session time remaining** — Colab free tier disconnects after ~90min idle / ~12hr max. Bannin tracks elapsed time and warns before timeout.
- **GPU type and reallocation** — Colab may switch your GPU (T4 → P100 → none) mid-session. Bannin detects GPU type changes and alerts if your GPU gets reassigned.
- **VRAM limits** — fixed per GPU type (T4=16GB, P100=16GB, A100=40GB). Bannin tracks VRAM usage and predicts OOM within these hard limits.
- **Ephemeral storage** — Colab's disk is temporary. Bannin monitors disk usage and triggers auto-checkpoint to Google Drive before session death.
...
- **Weekly GPU quota** — Kaggle gives ~30 hours/week of GPU time (P100). Bannin tracks cumulative usage and warns when quota is running low ("You have 4.2 hours of GPU left this week").
- **Session time limits** — Kaggle sessions max out at ~12 hours (CPU) or ~9 hours (GPU). Bannin counts down and alerts before timeout.
- **Internet access restrictions** — some competitions disable internet. Bannin detects this and warns if your code tries to download data.
- **Output storage limits** — Kaggle limits output to ~20GB. Bannin monitors output size and warns before you hit the cap.
- **Accelerator availability** — TPU and GPU availability fluctuates. Bannin detects your assigned accelerator type and alerts on changes.

### LLM Monitoring

This is a core differentiator — Bannin treats LLMs as systems that need monitoring, just like servers.
...
- Two integration patterns: `with bannin.track(): client.chat.completions.create(...)` or `client = bannin.wrap(OpenAI())`
...
- Conversation health score — long conversations get slower and dumber, Bannin detects this
...
Bannin's moat: unified relay (one dashboard for everything), intelligence levels (L0→L4), MCP distribution (organic adoption via Claude Code), and LLM-as-infrastructure monitoring (genuinely new category).

## Build Phases

### Phase 1a: Python Agent Core — ✅ COMPLETE
- Python package structure (pip install bannin)
...
- CLI entry point (bannin start / status / stop)
- bannin.watch() context manager
...
- bannin.wrap() for OpenAI, Anthropic, Google clients (+ auto-detection of OpenAI-compatible providers)
- bannin.track() context manager for named tracking scopes
...
- **pip-installable agent**: `pip install bannin` with decorator/context manager API
- **Cross-platform agents**: Python agent for ML/local, browser extension for web UIs
- **MCP integration**: Bannin as MCP server creates feedback loop — AI tools query their own health
...
- `Bannin.docx` — Original system architecture (3-component version)
- `Bannin_Senior_Engineer_Notes_Analysis.md.pdf` — Senior engineer feedback + strategic analysis
- `Bannin_Master_Architecture_v2.docx` — Unified master doc (5-component, competitive analysis, full roadmap)
